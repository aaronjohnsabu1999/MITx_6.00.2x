...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb, numTrials):
    ...
    """
    viruses = []
    avgPopSize = [0 for i in range(300)]
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        NoChildException if this virus particle does not reproduce.
        """
        def resistantToActiveDrugs():
            for drug in activeDrugs:
                if not self.resistances[drug]:
                    return False
            return True
        if resistantToActiveDrugs():
            if (random.random() < (self.maxBirthProb * (1 - popDensity))):
                mutatedResistances = copy.deepcopy(self.resistances)
                for drug in self.resistances:
                    if(random.random() < self.mutProb):
                        mutatedResistances[drug] = not(mutatedResistances[drug])
                newVirus = ResistantVirus(self.maxBirthProb, self.clearProb, mutatedResistances, self.mutProb)
                return newVirus
        raise NoChildException()
...
class TreatedPatient(Patient):
        ...
        for virus in self.viruses:
            for drug in drugResist:
                ...
                if drug in virus.getResistances().keys():
                    ...
                    resistant = False
                if (not resistant) and (virus in resistPop):
    ...
    def update(self):
        ...
        for i in range(self.getTotalPop()):
            try:
                self.viruses.append(self.viruses[i].reproduce(popDensity, self.adminDrugs))
            except NoChildException:
                continue
        return len(self.viruses)
